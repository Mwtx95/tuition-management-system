...
const PublicClassResults = () => {
  ...
  const fetchResults = async () => {
    try {
      setError("");
      const response = await api.get(`/results/class/${classId}/sheet/public/?exam_id=${examId}`);
      setSheet(response.data);
    } catch (err) {
      setError(err?.response?.data?.detail || "Failed to load public class results.");
    }
  };

  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-semibold mb-6">Public Class Results</h1>
            ...
            {sheet.rows.map((row) => (
              <tr key={row.student_id} className="border-t">
                ...
                {row.subjects.map((subject, index) => (
                  <td key={`${row.student_id}-${index}`} className="p-2">{subject.grade}</td>
                ))}
...
export default PublicClassResults;
